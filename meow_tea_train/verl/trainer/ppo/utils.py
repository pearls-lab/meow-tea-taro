...
# NOTE from meow-tea: A utility function to check if the environment is agentic (multi-turn).
# This is used in ray_trainer.py to determine if we need to use the specific agentic rollout manager.
# This allows flexibility in creating your own agentic system using minimal code modification.
def is_agentic(config: DictConfig) -> bool:
    """Given a config, is it agentic."""
    return config.agentic.environment.is_multiturn