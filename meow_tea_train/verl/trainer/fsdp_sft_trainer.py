...
from transformers import AutoConfig, AutoModelForCausalLM, PreTrainedModel, AutoProcessor, AutoModelForVision2Seq

import verl.utils.hdfs_io as hdfs_io
from verl.utils.dataset import SFTDataset
from verl.utils.dataset.multiturn_sft_dataset import MultiTurnSFTDataset
from verl.utils.dataset.chat_based_sft_dataset import ChatBasedSFTDataset
from verl.utils.dataset.rl_dataset import collate_fn as sft_collate_fn
...
class FSDPSFTTrainer:
    def __init__(
        ...
        self.eval_config = getattr(config.trainer, 'eval_config', {})
        self.do_exact_match_eval = self.eval_config.get('exact_match', False)
        self.max_response_len = self.eval_config.get('max_response_len', 256)  # Limit for efficiency
        self.temperature = self.eval_config.get('temperature', 0.0)  # Greedy decoding for consistency
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        # Prepare multimodal inputs if they exist
        multi_modal_inputs = batch.get("multi_modal_inputs", {})
        if multi_modal_inputs:
            for key, value in multi_modal_inputs.items():
                if isinstance(value, torch.Tensor):
                    multi_modal_inputs[key] = value.to(self.device_name)
    ...
    def validation_step(self, batch: TensorDict):
        ...
        with torch.no_grad():
            ...
            # pred_texts = self.generate_sequence(prompt_ids=batch["prompt_ids"])
            # response_ids = batch["response_ids"]
            # target_texts = []
            # for i in range(response_ids.shape[0]):
            #     target_texts.append(self.tokenizer.decode(response_ids[i], skip_special_tokens=True))
            # exact_match_score = self.compute_exact_match(pred_texts, target_texts)
    ...
    def _convert_right_to_left_padding(self, input_ids, attention_mask):
        ...
        # Create new tensors for left-padded sequences
        left_padded_input_ids = torch.full_like(input_ids, self.tokenizer.pad_token_id)
        left_padded_attention_mask = torch.zeros_like(attention_mask)
        
        for i in range(batch_size):
            # Find actual sequence length (number of non-padding tokens)
            actual_length = torch.sum(attention_mask[i]).item()
            
            if actual_length > 0:
                # Get the actual tokens (non-padding)
                actual_tokens = input_ids[i][:actual_length]
                
                # Place them at the end (left-padded)
                start_idx = seq_len - actual_length
                left_padded_input_ids[i, start_idx:] = actual_tokens
                left_padded_attention_mask[i, start_idx:] = 1
        
        return left_padded_input_ids, left_padded_attention_mask

    def generate_sequence(self, prompt_ids):
        # Save original padding side and set to left for generation
        original_padding_side = self.tokenizer.padding_side
        self.tokenizer.padding_side = 'left'
        
        with torch.no_grad():
            # Generate with the properly padded inputs
            ...
            outputs = outputs[:, prompt_ids.shape[1]:]
            output_texts = []
            for i in range(outputs.shape[0]):
                output_texts.append(self.tokenizer.decode(outputs[i], skip_special_tokens=True))

            self.tokenizer.padding_side = original_padding_side
            return output_texts
    
    def compute_exact_match(self, predictions, targets):
            ...
            for data in tqdm(
                ...
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    ...
                    for val_data in self.val_dataloader:
                        ...
                        val_exact_match_score, val_loss = self.validation_step(val_data)
                        val_scores.append(val_exact_match_score)
                        val_losses.append(val_loss)
                    if rank == 0:
                        val_score = sum(val_scores) / len(val_scores)
                        val_loss = torch.mean(torch.stack(val_losses))
                        metric = {"val/loss": val_loss.detach().item(), "val/exact_match": val_score}
                        tracking.log(data=metric, step=global_step)
...
def run_sft(config):
    ...
    processor = None
    if config.data.get("chat_based", {}).get("enable", False) and config.data.get("chat_based", {}).get(
        "return_multi_modal_inputs", False
    ):
        processor = AutoProcessor.from_pretrained(local_model_path, trust_remote_code=config.model.trust_remote_code)
...
def create_sft_dataset(data_paths, data_config, tokenizer, processor):
    ...
    # Then check if chat-based dataset (new multiturn)
    elif data_config.get("chat_based", {}).get("enable", False):
        dataset_cls = ChatBasedSFTDataset
